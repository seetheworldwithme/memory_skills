...
export function SkillsPage({ api }: { api: ApiClient }) {
  ...
  return <section className="workspace"><header className="page-head"><div><p className="eyebrow">SKILL / EXECUTABLE KNOWLEDGE</p><h1>Skill 库</h1><p>管理可复用的执行方法、版本与审核状态。</p></div><div className="page-actions"><button className="secondary" onClick={() => void runProposal()} disabled={proposing}><Sparkles size={17} aria-hidden="true" />{proposing ? "生成中…" : "从证据生成草稿"}</button><button className="primary" onClick={() => setCreating(true)}><Plus size={17} aria-hidden="true" />新建 Skill</button></div></header>{proposalNote && <div className="info-banner" role="status">{proposalNote}</div>}{error && <div className="error-banner" role="alert">{error}</div>}<div className="split-panel"><aside className="asset-list"><label className="search"><Search size={16} aria-hidden="true" /><input aria-label="搜索 Skill" type="search" autoComplete="off" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="搜索名称或描述" /></label><div className="list-meta"><span>{filtered.length} 个 Skill</span><span>按更新时间</span></div><div className="list-scroll">{loading && <Empty text="正在加载 Skill…" />}{!loading && filtered.length === 0 && <Empty text="还没有 Skill，先创建一份可执行方法。" />}{filtered.map((item) => <button key={item.id} className={`asset-row ${selectedId === item.id ? "is-active" : ""}`} onClick={() => setSelectedId(item.id)}><div className="asset-row-top"><span className="layer">V{item.version}</span><StatusBadge status={item.status} /></div><strong className="skill-name">{item.name}</strong><p>{item.description}</p><small>{formatDateTime(item.updatedAt)}</small></button>)}</div></aside><main className="detail-pane">{selected ? <SkillDetail key={`${selected.id}:${selected.version}`} item={selected} api={api} onTransition={async (target) => { try { await api.transitionSkill(selected.id, target); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "状态更新失败"); } }} onReload={load} /> : <Empty text="选择一个 Skill 查看 SKILL.md" />}</main></div>{creating && <SkillDialog onClose={() => setCreating(false)} onSubmit={async (input) => { await api.createSkill(input); setCreating(false); await load(); }} />}</section>;
}
...
function SkillDetail({ item, api, onTransition, onReload }: {
  ...
  const [confirming, setConfirming] = useState<"rejected" | "archived">(); const [pending, setPending] = useState(false);
  ...
  const runTransition = async (target: "verified" | "rejected" | "archived") => { try { setPending(true); await onTransition(target); } finally { setPending(false); setConfirming(undefined); } };
  ...
  return <div className="detail-content"><div className="detail-title"><div><span className="mono-id">SKILL.md · VERSION {item.version}</span><h2 className="skill-heading">{item.name}</h2><p>{item.description}</p></div><StatusBadge status={item.status} /></div>
    {note && <div className="error-banner" role="alert">{note}</div>}
    ...
    <FeedbackBar key={item.id} api={api} assetKind="skill" assetId={item.id} /></div>;
}
...
function SkillDialog({ onClose, onSubmit }: { onClose: () => void; onSubmit: (input: { name: string; description: string }) => Promise<void> }) { const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [saving, setSaving] = useState(false); const [submitError, setSubmitError] = useState<string>(); return <Modal eyebrow="NEW SKILL" title="创建 Skill 草稿" onClose={onClose} onSubmit={(event) => { event.preventDefault(); setSaving(true); setSubmitError(undefined); void onSubmit({ name, description }).catch((reason) => setSubmitError(reason instanceof Error ? reason.message : "Skill 创建失败")).finally(() => setSaving(false)); }}><label>名称<input aria-label="名称" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" title="只使用小写字母、数字和连字符，例如 review-code" autoComplete="off" spellCheck={false} value={name} onChange={(e) => setName(e.target.value)} placeholder="review-code" /><small>使用小写字母、数字和连字符</small></label><label>描述<textarea aria-label="描述" required value={description} onChange={(e) => setDescription(e.target.value)} placeholder="一句话说明它解决什么问题" /></label>{submitError && <div className="login-error" role="alert">{submitError}</div>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>取消</button><button type="submit" className="primary" disabled={saving}>{saving ? "创建中…" : "创建草稿"}</button></div></Modal>; }

function Empty({ text }: { text: string }) { return <div className="empty"><FileCode2 aria-hidden="true" /><p>{text}</p></div>; }
